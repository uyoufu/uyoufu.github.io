...
通过引入原型模式，Sunny软件公司OA系统支持工作周报的快速克隆，极大提高了工作周报的编写效率，受到员工的一致好评。但有员工又发现一个问题，有些工作周报带有附件，例如经理助理“小龙女”的周报通常附有本周项目进展报告汇总表、本周客户反馈信息汇总表等，如果使用上述原型模式来复制周报，周报虽然可以复制，但是周报的附件并不能复制，这是由于什么原因导致的呢？
...
也就是说原型对象和克隆对象的成员变量指向相同的内存地址。简单来说，在浅克隆中，当对象被复制时只复制它本身和其中包含的值类型的成员变量，而引用类型的成员对象并没有复制，如图所示：

![](https://i.loli.net/2020/07/23/dcj8pAsMfvtYH7B.png)

在Java语言中，通过覆盖Object类的clone()方法可以实现浅克隆。为了让大家更好地理解浅克隆和深克隆的区别，我们首先使用浅克隆来实现工作周报和附件类的复制，其结构如图所示：

![](https://i.loli.net/2020/07/23/qnlOX79SiYgpJEQ.png)
...
![](https://i.loli.net/2020/07/23/Vf8yIXO7kUcMuim.png)
...
![](https://i.loli.net/2020/07/23/rMn7HdRwlDjcPsB.png)
...
在原型管理器中针对抽象原型类进行编程，以便扩展。其结构如图所示：
![](https://i.loli.net/2020/07/23/YTecbf9K41M5zmi.png)
...
![](https://i.loli.net/2020/07/23/TQcSnehIBOUNKy9.png)
...
Sunny公司的开发人员通过分析发现，游戏角色是一个复杂对象，它包含性别、脸型等多个组成部分，不同的游戏角色其组成部分有所差异，如图所示：

![](https://i.loli.net/2020/07/25/YPwdi341amVu7go.png)
...
![](https://i.loli.net/2020/07/25/cK9Yi5pQEImoWkf.png)
...
Sunny公司开发人员决定使用建造者模式来实现游戏角色的创建，其基本结构如图所示：

![](https://i.loli.net/2020/07/25/QeLA3nvCJUlSxf4.png)
...
以上两种对Director类的省略方式都不影响系统的灵活性和可扩展性，同时还简化了系统结构，但加重了抽象建造者类的职责，如果construct()方法较为复杂，待构建产品的组成部分较多，建议还是将construct()方法单独封装在Director中，将产品的创建和生成逻辑分开，这样做更符合“单一职责原则”。
...
> 我觉得钩子方法可有可无。既然是在子类DevilBuilder中重写钩子方法，那么其实可以直接重写buildHairstyle方法（比如空方法）。可以同样达到效果，还降低了复杂度。
> 
> 反正实现方法千万，最后都要回归到代码风格的7大标准中，要符合最基本的 **开闭原则**。
